...
use super::super::super::{common::*, cpu::*};
...
impl<C: PageTableConfig> SubTreeForgotGuard<C> {
    ...
    pub open spec fn wf(&self) -> bool {
        ...
        &&& forall |nid: NodeId| #[trigger]
            self.inner.dom().contains(nid) ==> {
                ...
                &&& self.childs_are_contained(nid, self.get_guard_inner(nid).pte_token->Some_0.value())
            }
    }

    pub open spec fn is_root(&self, nid: NodeId) -> bool {
        forall |_nid: NodeId| #[trigger]
            self.inner.dom().contains(_nid) ==>
    ...
    pub open spec fn childs_are_contained(&self, nid: NodeId, pte_array: PteArrayState) -> bool {
        &&& NodeHelper::is_not_leaf(nid) ==>
            forall |i: nat| 0 <= i < 512 ==> {
                #[trigger] pte_array.is_alive(i) ==>
                    self.inner.dom().contains(NodeHelper::get_child(nid, i))
            }
        &&& !NodeHelper::is_not_leaf(nid) ==>
            pte_array =~= PteArrayState::empty()
    }

    pub open spec fn childs_are_contained_constrained(&self, nid: NodeId, pte_array: PteArrayState, idx: nat) -> bool {
        &&& NodeHelper::is_not_leaf(nid) ==>
...
impl<C: PageTableConfig> SubTreeForgotGuard<C> {
            ...
            self.childs_are_contained(nid, res.pte_token->Some_0.value()),
    {
        ...
        assert forall |_nid: NodeId| #[trigger] self.inner.dom().contains(_nid) implies {
            ...
            if NodeHelper::is_not_leaf(_nid) {
                assert forall |i: nat| 
                    0 <= i < 512 && 
                    #[trigger] self.get_guard_inner(_nid).pte_token->Some_0.value().is_alive(i) 
                implies {
                    ...
                    if _nid != nid {
                        ...
                        assert(NodeHelper::get_child(_nid, i) != nid) by {
                            ...
                            NodeHelper::lemma_is_child_nid_increasing(_nid, NodeHelper::get_child(_nid, i));
                            NodeHelper::lemma_is_child_implies_in_subtree(_nid, NodeHelper::get_child(_nid, i));
                            NodeHelper::lemma_in_subtree_iff_in_subtree_range(_nid, NodeHelper::get_child(_nid, i));
                        };
                    }
                }
            }
        ...
        if NodeHelper::is_not_leaf(nid) {
            assert forall |i: nat| 
                0 <= i < 512 && 
                #[trigger] res.0.pte_token->Some_0.value().is_alive(i) 
            implies {
                self.inner.dom().contains(NodeHelper::get_child(nid, i))
            } by {
    ...
    pub open spec fn union_spec(self, other: Self) -> Self {
        Self {
            inner: self.inner.union_prefer_right(other.inner)
        }
    }
    ...
    pub open spec fn get_sub_tree_dom(&self, nid: NodeId) -> Set<NodeId> {
        self.inner.dom().intersect(Set::new(
            ...
            res.is_root_and_contained(nid),
    {
        ...
        assert forall |_nid: NodeId| #[trigger]
            res.inner.dom().contains(_nid)
        implies {
            res.childs_are_contained(
                _nid, 
                res.get_guard_inner(_nid).pte_token->Some_0.value(),
            )
        } by {
            assert(NodeHelper::in_subtree_range(nid, _nid));
            if NodeHelper::is_not_leaf(_nid) {
