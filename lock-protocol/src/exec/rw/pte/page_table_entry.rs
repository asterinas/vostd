...
impl PageTableEntry {
    ...
    pub open spec fn format_property_spec(entry: usize) -> PageProperty {
        let flags: u8 =
            if entry & PageTableFlags::PRESENT().bits() == 0 {0} else { PageFlags::R().bits() } |
            if entry & PageTableFlags::WRITABLE().bits() == 0 {0} else { PageFlags::W().bits() } |
            if entry & PageTableFlags::ACCESSED().bits() == 0 {0} else { PageFlags::ACCESSED().bits() } |
            if entry & PageTableFlags::DIRTY().bits() == 0 {0} else { PageFlags::DIRTY().bits() } |
            if !entry & PageTableFlags::NO_EXECUTE().bits() == 0 {0} else { PageFlags::X().bits() };
        let priv_flags: u8 =
            if entry & PageTableFlags::USER().bits() == 0 {0} else { PrivilegedPageFlags::USER().bits() } |
            if entry & PageTableFlags::GLOBAL().bits() == 0 {0} else { PrivilegedPageFlags::GLOBAL().bits() };
        ...
        returns Self::format_property_spec(entry)
    {
        ...
        let flags =
            ...
            parse_flags!(entry, PageTableFlags::WRITABLE(), PageFlags::W()) |
            parse_flags!(entry, PageTableFlags::ACCESSED(), PageFlags::ACCESSED()) |
            parse_flags!(entry, PageTableFlags::DIRTY(), PageFlags::DIRTY()) |
            parse_flags!(!entry, PageTableFlags::NO_EXECUTE(), PageFlags::X());
        let priv_flags =
            parse_flags!(entry, PageTableFlags::USER(), PrivilegedPageFlags::USER()) |
            parse_flags!(entry, PageTableFlags::GLOBAL(), PrivilegedPageFlags::GLOBAL());
        let cache = Self::format_cache(entry);
        PageProperty {
            flags: PageFlags::from_bits(flags as u8),
            cache,
