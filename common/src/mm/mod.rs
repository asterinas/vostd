...
pub proof fn lemma_page_size_adjacent_levels<C: PagingConstsTrait>(level: PagingLevel)
    by (nonlinear_arith)
    requires
        1 < level <= C::NR_LEVELS() + 1,
    ensures
...
// geometric sequence.
pub proof fn lemma_page_size_geometric<C: PagingConstsTrait>(i: PagingLevel, j: PagingLevel)
    by (nonlinear_arith)
    requires
        1 <= i <= j <= C::NR_LEVELS() + 1,
    ensures
