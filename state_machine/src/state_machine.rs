...
impl<State, Input, ActionInput, Output, Step> StateMachine<
    State,
    Input,
    ActionInput,
    Output,
...
> {
    ...
    pub open spec fn next_result(self, input: Input, s: State) -> ActionResult<State, Output> {
        if exists|step|
            (#[trigger] (self.step_to_action)(step).precondition)(
                (self.action_input)(step, input),
                s,
            ) {
            let witness_step = choose|step|
                (#[trigger] (self.step_to_action)(step).precondition)(
                    (self.action_input)(step, input),
                    s,
                );
            ...
            ActionResult::Enabled(
                (action.transition)(action_input, s).0,
                (action.transition)(action_input, s).1,
            )
        } else {
    ...
    pub open spec fn next_action_result(
        self,
        action: Action<State, ActionInput, Output>,
        action_input: ActionInput,
        s: State,
    ) -> ActionResult<State, Output> {
...
impl<State, MessageOps> NetworkStateMachine<State, MessageOps> {
    ...
    pub open spec fn next_result(self, msg_ops: MessageOps, s: State) -> ActionResult<State, ()> {
        if (self.deliver.precondition)(msg_ops, s) {
            ActionResult::Enabled(
                (self.deliver.transition)(msg_ops, s).0,
                (self.deliver.transition)(msg_ops, s).1,
            )
        } else {
            ActionResult::Disabled
        }
