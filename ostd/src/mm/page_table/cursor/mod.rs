...
impl<'rcu, C: PageTableConfig, A: InAtomicMode> Cursor<'rcu, C, A> {
            ...
            regions.inv(),
    {
            ...
            decreases self.level,
        {
            ...
            assert(regions.slot_owners.contains_key(
                frame_to_index(
                    meta_to_frame(
                        owner.continuations[(owner.level
                            - 1) as int].entry_own.node.unwrap().as_node.meta_perm.addr,
                    ),
                ),
            )) by { admit() };
    ...
    fn cur_entry(&mut self) -> (res: Entry<'rcu, C>)
        requires
            ...
            regions.slot_owners.contains_key(
                frame_to_index(
                    meta_to_frame(
                        old(owner).continuations[(old(owner).level
                            - 1) as int].entry_own.node.unwrap().as_node.meta_perm.addr,
                    ),
                ),
            ),
...
impl<'rcu, C: PageTableConfig, A: InAtomicMode> CursorMut<'rcu, C, A> {
    ...
    #[verus_spec(
        with Tracked(owner): Tracked<&mut CursorOwner<'rcu, C>>,
            Tracked(entry_own): Tracked<EntryOwner<'rcu, C>>,
            Tracked(regions): Tracked<&mut MetaRegionOwners>
    )]
            ...
            entry_own.inv(),
    {
        ...
        #[verus_spec(with Tracked(owner), Tracked(entry_own))]
            ...
            self.inner.wf(*owner),
    {
        ...
        match old {
            ...
            Child::PageTable(pt) => {
                ...
                let pt_val = locked_pt.borrow(
                    Tracked(
                        &owner.continuations[owner.level as int].entry_own.node.unwrap().guard_perm,
                    ),
                );
