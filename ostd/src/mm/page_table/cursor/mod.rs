...
impl<'rcu, C: PageTableConfig, A: InAtomicMode> Cursor<'rcu, C, A> {
        ...
        proof {
            ...
            assert forall|i: int| 0 <= i < NR_ENTRIES implies pt_own.0.children[i] is Some by {
                if pt_own.0.children[i] is None {
                    assert(<EntryOwner<C> as TreeNodeValue<INC_LEVELS>>::rel_children(
                        pt_own.0.value,
                        i,
                        None,
                    ));
            ...
            invariant
                ...
                forall|idx: usize|
                    ...
                    old(regions).slot_owners.contains_key(idx) ==> {
                        &&& regions.slot_owners[idx].path_if_in_pt == old(
                            regions,
                        ).slot_owners[idx].path_if_in_pt
                        ...
                        &&& regions.slot_owners[idx].inner_perms.storage.id() == old(
                            regions,
                        ).slot_owners[idx].inner_perms.storage.id()
            ...
            match cur_child {
                        ...
                        proof {
                            ...
                            if !split_happened {
                                ...
                                assert(old(owner)@.mappings.filter(
                                    |m: Mapping|
                                        va_before_move <= m.va_range.start < va_before_move
                                    ...
                                    assert forall|m: Mapping|
                                        ...
                                        if old(owner)@.mappings.contains(m) && old(self).va
                                            ...
                                            if m.va_range.start < va_before_move {
                                                ...
                                                assert(m.va_range.start < va_before_move
                                                    + cur_slot_size as usize);
                                                assert(old(owner)@.mappings.filter(
                                                    |m2: Mapping|
                                                        va_before_move <= m2.va_range.start
                                                            < va_before_move
                                                            + cur_slot_size as usize,
                                                ).contains(m));
                                            }
                                    ...
                                    if old(owner)@.mappings.contains(m) && old(self).va
                                        ...
                                        if m.va_range.start < va_before_move {
                                            ...
                                            assert(old(owner)@.mappings.filter(
                                                |m2: Mapping|
                                                    va_before_move <= m2.va_range.start
                                                        < va_before_move + cur_slot_size as usize,
                                            ).contains(m));
                                        }
                                    }
                                };
                ...
                ChildRef::Frame(_, _, _) => {
                    ...
                    assume(child_owner.value.is_frame() && parent_owner.level > 1 ==> {
                        ...
                        forall|j: usize|
                            #![trigger frame_to_index((pa + j * page_size(sub_level)) as usize)]
                            j < NR_ENTRIES ==> {
        ...
        proof {
            if self.level >= self.guard_level {
                ...
                AbstractVaddr::wrapped_nonzero_at_level(
                    ...
                    start_level as int,
                    self.level as int,
                    abs_va_down.index[self.level as int - 1],
                );
            }
        }
        assert(index != 0);
            ...
            *regions == *old(regions),
    {
        ...
        proof {
            ...
            assert forall|i: int|
                ...
                owner.level - 1 <= i
                    < NR_LEVELS implies owner.continuations[i].guard_perm.value().inner.inner@.ptr.addr()
                != dropped_addr by {};
    ...
    fn push_level(&mut self, child_pt: PPtr<PageTableGuard<'rcu, C>>)
        ...
        ensures
            ...
            owner.nodes_locked(*guards),
            owner.metaregion_sound(*regions),
            old(owner).metaregion_correct(*regions) ==> owner.metaregion_correct(*regions),
            self.inv(),
            self.wf(*owner),
            self.guard_level == old(self).guard_level,
            self.barrier_va == old(self).barrier_va,
...
impl<'rcu, C: PageTableConfig, A: InAtomicMode> CursorMut<'rcu, C, A> {
        ...
        match res {
            Ok((inner_cursor, tracked_owner)) => {
                proof {
                    ...
                    assert(CursorMut::<C, A> { inner: inner_cursor }.inner.invariants(
                        *tracked_owner,
                        *regions,
                        *guards,
                    ));
    ...
    fn map_branch_none(&mut self, cur_entry: &mut Entry<'rcu, C>, rcu_guard: &'rcu A) {
        ...
        proof {
            ...
            assert forall|i: int|
                #![trigger owner.continuations[i]]
                owner.level <= i < NR_LEVELS implies owner.continuations[i].map_children(g_unlocked)
            ...
            assert(owner.continuations[owner.level - 1].idx == owner0.continuations[owner0.level
                - 1].idx);
            ...
            assert(owner.continuations[owner.level - 1].path() == owner0.continuations[owner0.level
                - 1].path());
            ...
            assert(owner.continuations[owner.level - 1].guard_perm.value().inner.inner@.ptr.addr()
                == owner0.continuations[owner0.level
                - 1].guard_perm.value().inner.inner@.ptr.addr());
            ...
            if owner0.metaregion_correct(*old(regions)) {
                ...
                assert(cont_final.children[idx].unwrap().tree_predicate_map(
                    cont_final.path().push_tail(idx as usize),
                    g_ptp,
                )) by {
        ...
        proof {
            ...
            assert forall|idx: usize|
                old(regions).slot_owners[idx].inner_perms.ref_count.value()
                    != REF_COUNT_UNUSED implies (#[trigger] regions.slot_owners[idx]) == old(
                regions,
            ).slot_owners[idx] by {};
    ...
    pub fn map_loop(&mut self, level: PagingLevel, rcu_guard: &'rcu A) {
        ...
        while self.inner.level != level
            invariant
                ...
                (self.inner.level <= owner0.level && owner0.cur_entry_owner().is_absent())
                    ==> owner.cur_entry_owner().is_absent(),
    ...
    #[verus_spec(res =>
        ...
        ensures
            ...
            old(owner).metaregion_correct(*old(regions))
                && Self::item_not_mapped(item, *old(regions))
                ==> owner.metaregion_correct(*regions),
    ...
    pub fn map(&mut self, item: C::Item) -> (res: Result<(), PageTableFrag<C>>) {
        ...
        proof {
            ...
            if owner1.metaregion_correct(regions_before_new_child) && Self::item_not_mapped(
                item,
                *old(regions),
            ) {
                ...
                assert(regions_before_new_child.slot_owners[pa_idx].path_if_in_pt is None) by {
                    ...
                    assert(regions_before_new_child.slot_owners[pa_idx] == old(
                        regions,
                    ).slot_owners[pa_idx]);
        ...
        proof {
            assert(self.inner.va >= self.inner.barrier_va.end || owner.in_locked_range()) by {
            ...
            assert forall|idx: usize|
                idx != pa_idx2 && old(regions).slot_owners[idx].inner_perms.ref_count.value()
                    != REF_COUNT_UNUSED implies #[trigger] regions.slot_owners[idx].path_if_in_pt
                == old(regions).slot_owners[idx].path_if_in_pt by {
    ...
    #[verus_spec(res =>
        ...
        ensures
            ...
            res is Some && res.unwrap() is Mapped ==>
                forall |m2: Mapping|
                    #[trigger] owner@.mappings.contains(m2) && old(self).inner.va <= m2.va_range.start
    ...
    pub fn take_next(&mut self, len: usize) -> (r: Option<PageTableFrag<C>>) {
        ...
        proof {
            ...
            if frag.unwrap() is Mapped {
                assume(old(owner)@.mappings.filter(
                    |m: Mapping| old_va <= m.va_range.start < frag.unwrap()->Mapped_va,
                ) =~= Set::<Mapping>::empty());
            }
            if frag.unwrap() is StrayPageTable {
                assume(old(owner)@.mappings.filter(
                    |m: Mapping| old_va <= m.va_range.start < frag.unwrap()->StrayPageTable_va,
                ) =~= Set::<Mapping>::empty());
            }
            ...
            if frag.unwrap() is Mapped {
                assume(forall|m: Mapping| #[trigger]
                    owner@.mappings.contains(m) && old_va <= m.va_range.start && m.va_range.start
                        < frag.unwrap()->Mapped_va ==> old(owner)@.mappings.contains(m));
            }
            if frag.unwrap() is StrayPageTable {
                assume(forall|m: Mapping| #[trigger]
                    owner@.mappings.contains(m) && old_va <= m.va_range.start && m.va_range.start
                        < frag.unwrap()->StrayPageTable_va ==> old(owner)@.mappings.contains(m));
            }
            ...
            if frag.unwrap() is Mapped {
                assume(owner@.mappings.filter(
                    |m: Mapping| frag.unwrap()->Mapped_va <= m.va_range.start < self.inner.va,
                ) =~= Set::<Mapping>::empty());
            }
            if frag.unwrap() is StrayPageTable {
                assume(owner@.mappings.filter(
    ...
    fn replace_cur_entry(&mut self, new_child: Child<C>) -> (res: Option<PageTableFrag<C>>)
        ...
        ensures
            ...
            old(owner).metaregion_correct(*old(regions)) && (new_owner.value.is_absent() || old(
                regions,
            ).slot_owners[frame_to_index(
                new_owner.value.meta_slot_paddr().unwrap(),
            )].path_if_in_pt is None) ==> owner.metaregion_correct(*regions),
            self.inner.barrier_va == old(self).inner.barrier_va,
            owner.va == old(owner).va,
            self.inner.va == old(self).inner.va,
                ...
                ).slot_owners[idx].path_if_in_pt,
    {
        ...
        let ghost write_path = !old_child_pre_replace.is_node() && owner0.metaregion_correct(
            regions0,
        ) && pre_new_owner_value.meta_slot_paddr() is Some && regions0.slot_owners[frame_to_index(
            pre_new_owner_value.meta_slot_paddr().unwrap(),
        )].path_if_in_pt is None;
        ...
        proof {
            ...
            assert forall|i: int|
                #![trigger owner.continuations[i]]
                owner0.level <= i < NR_LEVELS implies { owner.continuations[i].map_children(g_sound)
            } by {
                ...
                if old_child_pre_replace.is_node() {
                    ...
                    let ghost new_idx2 = frame_to_index(
                        pre_new_owner_value.meta_slot_paddr().unwrap(),
                    );
            ...
            assert(new_owner.tree_predicate_map(child_path, g_sound)) by {
                assert forall|lv: nat, p: TreePath<NR_ENTRIES>|
                    lv < INC_LEVELS implies #[trigger] g_sound(
                    <EntryOwner<C> as TreeNodeValue<INC_LEVELS>>::default(lv),
                    p,
                ) by {};
            ...
            if owner0.metaregion_correct(regions0) && (pre_new_owner_value.is_absent()
                ...
                assert(new_owner.value.is_node() ==> PageTableOwner::<C>::path_tracked_pred(
                    *regions,
                ...
                assert(OwnerSubtree::implies(
                    ...
                    assert forall|e: EntryOwner<C>, p: TreePath<NR_ENTRIES>|
                        e.inv() && f_path(e, p) && f_neq_new(e, p) implies #[trigger] g_path(
                        e,
                        p,
                    ) by {
                        if e.meta_slot_paddr() is Some {
                ...
                assert forall|i: int|
                    #![trigger owner.continuations[i]]
                    owner0.level <= i < NR_LEVELS implies {
                ...
                assert(new_owner.tree_predicate_map(child_path, g_path)) by {
                    assert forall|lv: nat, p: TreePath<NR_ENTRIES>|
                        lv < INC_LEVELS implies #[trigger] g_path(
                        <EntryOwner<C> as TreeNodeValue<INC_LEVELS>>::default(lv),
                        p,
                    ) by {};
                ...
                proof {
                    ...
                    assert(forall|m: Mapping|
                        owner.view_mappings().contains(m)
                            <==> owner_before_dfs.view_mappings().contains(m));
                }
        ...
        assert(forall|idx: usize|
            ...
            (new_owner.value.is_absent() || idx != frame_to_index(
                ...
                == regions0.slot_owners[idx].path_if_in_pt) by {
            assert(forall|i: usize|
                #![trigger regions.slot_owners[i].path_if_in_pt]
                regions.slot_owners[i].path_if_in_pt
                    == regions_after_replace.slot_owners[i].path_if_in_pt);
        };
