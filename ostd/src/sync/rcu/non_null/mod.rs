...
pub unsafe trait NonNullPtrRef<'a>: NonNullPtr {
    ...
    /// Converts a shared reference to a raw pointer.
    fn ref_as_raw(ptr_ref: Self::Ref) -> ((res_ptr, perm): (
        NonNull<Self::Target>,
        Tracked<Self::RefPermission>,
    ))
        ensures
...
unsafe impl<T: 'static> NonNullPtr for Arc<T> {
    ...
    fn into_raw(self) -> (NonNull<Self::Target>, Tracked<Self::Permission>) {
        ...
        let ptr = (#[verus_spec(with => Tracked(perm))]
        arc_into_raw(self)).cast_mut();
        assume(ptr.addr() % (1usize << Self::ALIGN_BITS) == 0);
        ...
        unsafe {
            ArcRef {
                inner: ManuallyDrop::new(
                    #[verus_spec(with perm)]
                    arc_from_raw(raw.as_ptr()),
                ),
                _marker: PhantomData,
            }
        }
