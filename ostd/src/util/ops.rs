...
#[verus_spec(ret =>
    requires
        ...
        vstd::laws_cmp::obeys_cmp::<T>(),
    ensures
        iterator_model_valid(ret),
        iterator_remaining::<_, Range<T>>(ret) == range_difference_seq(*a, *b),
        iterator_remaining::<_, Range<T>>(ret).len() <= 2,
        forall|i: int|
            0 <= i < iterator_remaining::<_, Range<T>>(ret).len() ==> (
            #[trigger] iterator_remaining::<_, Range<T>>(ret)[i]).start.is_lt(
                &iterator_remaining::<_, Range<T>>(ret)[i].end,
            ),
        forall|i: int|
            0 <= i < iterator_remaining::<_, Range<T>>(ret).len() - 1 ==> (
            #[trigger] iterator_remaining::<_, Range<T>>(ret)[i]).end.is_le(
                &iterator_remaining::<_, Range<T>>(ret)[i + 1].start,
            ),
        seq_range_union(iterator_remaining::<_, Range<T>>(ret)) == range_as_set(*a).difference(
            range_as_set(*b),
        ),
)]
