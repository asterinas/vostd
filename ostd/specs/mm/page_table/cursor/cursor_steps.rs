...
impl<'rcu, C: PageTableConfig> CursorOwner<'rcu, C> {
            ...
            self.push_level_owner(guard)@.mappings == self@.mappings,
    {
        ...
        assert(modified_cont.children == taken.children) by {
            assert forall|j: int|
                0 <= j < modified_cont.children.len() implies modified_cont.children[j]
                == taken.children[j] by {
            ...
            self.push_level_owner(guard).inv(),
    {
        ...
        assert(modified_cont.all_but_index_some()) by {
            ...
            assert forall|i: int|
                0 <= i < modified_cont.idx implies modified_cont.children[i] is Some by {
                assert(modified_cont.children[i] == old_cont.children[i]);
            };
            assert forall|i: int|
                modified_cont.idx < i < NR_ENTRIES implies modified_cont.children[i] is Some by {
                assert(modified_cont.children[i] == old_cont.children[i]);
            };
        };

        assert(forall|i: int|
            new_owner.level <= i < NR_LEVELS ==> {
        ...
        assert(modified_cont.inv_children()) by {
            assert forall|i: int|
                0 <= i < modified_cont.children.len()
                    && #[trigger] modified_cont.children[i] is Some implies modified_cont.children[i].unwrap().inv() by {
            ...
            self.push_level_owner(guard).metaregion_sound(regions),
    {
        ...
        assert forall|i: int|
            #![trigger new_owner.continuations[i]]
            new_owner.level - 1 <= i
                < new_owner.guard_level implies new_owner.continuations[i].node_locked(guards) by {
            if i == self.level - 2 {
                assert(new_owner.continuations[i] == child_cont);
        ...
        assert(new_owner.metaregion_sound(regions)) by {
            assert forall|i: int| #![auto] new_owner.level - 1 <= i < NR_LEVELS implies {
            ...
            self.pop_level_owner().0.inv(),
    {
        ...
        assert(new_owner.level <= 4 ==> {
            ...
            &&& new_owner.continuations[3].inv()
            &&& new_owner.continuations[3].level() == 4
            &&& new_owner.continuations[3].entry_own.parent_level
                == 5
        ...
        assert(new_owner.level <= 3 ==> {
            ...
            &&& new_owner.continuations[2].level() == 3
            &&& new_owner.continuations[2].entry_own.parent_level == 4
            &&& new_owner.in_locked_range() ==> new_owner.va.index[2]
                == new_owner.continuations[2].idx
            &&& new_owner.continuations[2].guard.inner.inner@.ptr.addr()
                != new_owner.continuations[3].guard.inner.inner@.ptr.addr()
            &&& new_owner.continuations[2].path() == new_owner.continuations[3].path().push_tail(
                new_owner.continuations[3].idx as usize,
        ...
        assert(new_owner.level <= 2 ==> {
            ...
            &&& new_owner.continuations[1].level() == 2
            &&& new_owner.continuations[1].entry_own.parent_level == 3
            &&& new_owner.in_locked_range() ==> new_owner.va.index[1]
                == new_owner.continuations[1].idx
            &&& new_owner.continuations[1].guard.inner.inner@.ptr.addr()
                != new_owner.continuations[2].guard.inner.inner@.ptr.addr()
            &&& new_owner.continuations[1].guard.inner.inner@.ptr.addr()
                != new_owner.continuations[3].guard.inner.inner@.ptr.addr()
        ...
        decreases NR_LEVELS - self.level,
    {
        ...
        if self.index() + 1 < NR_ENTRIES {
            ...
            assert(new_state.continuations[self.level - 1].idx == self.continuations[self.level
                - 1].idx + 1);
            ...
            assert(popped2.max_steps() == self.max_steps_partial(lp1) + Self::max_steps_subtree(
                lp1,
            ));
            ...
            assert(popped2.move_forward_owner_spec().max_steps() + Self::max_steps_subtree(lp1)
                <= popped2.max_steps());
            ...
            assert(popped2.move_forward_owner_spec().max_steps() <= self.max_steps_partial(lp1));
        ...
        decreases NR_LEVELS - self.level,
    {
        if self.level == self.guard_level {
            if self.index() + 1 < NR_ENTRIES {
                ...
                assert(vstd_extra::arithmetic::nat_align_down(self_va, ps) + ps
                    <= usize::MAX as nat);
        ...
        if self.index() + 1 < NR_ENTRIES {
            ...
            if !popped.popped_too_high {
                ...
                assert(vstd_extra::arithmetic::nat_align_down(popped_va, ps_p) + ps_p
                    <= usize::MAX as nat);
                popped.va.align_up_advances_general(popped.level as int);
                assert(nat_align_down(inc_p_va, ps_p) == vstd_extra::arithmetic::nat_align_down(
                    popped_va,
                    ps_p,
                ) + ps_p);
        ...
        decreases NR_LEVELS - self.level,
    {
        ...
        if self.index() + 1 < NR_ENTRIES {
            ...
            assert(result == self.move_forward_owner_spec());
            assert(result.view_mappings() == self.view_mappings());
            assert(self.move_forward_owner_spec()@.mappings == self@.mappings);
        } else if self.level < NR_LEVELS {
            let popped = self.pop_level_owner().0;
            ...
            // level == NR_LEVELS, last index: move_forward_owner_spec only changes va/popped_too_high
