...
impl<'rcu, C: PageTableConfig> CursorOwner<'rcu, C> {
    pub proof fn protect_preserves_cursor_inv_metaregion(
        ...
        requires
            ...
            other.continuations[self.level - 1].guard.inner.frame().ptr.addr()
                == self.continuations[self.level - 1].guard.inner.frame().ptr.addr(),
    ...
    pub proof fn map_branch_none_inv_holds(self, owner0: Self)
        requires
            ...
            self.continuations[self.level - 1].guard.inner.frame().ptr.addr()
                == owner0.continuations[owner0.level - 1].guard.inner.frame().ptr.addr(),
