...
impl<C: PageTableConfig> PageTableOwner<C> {
    ...
    pub proof fn lemma_view_rec_contains_intro(self, path: TreePath<NR_ENTRIES>, m: Mapping, i: int)
        requires
            self.0.inv(),
            path.len() < INC_LEVELS - 1,
            self.0.value.is_node(),
    ...
    pub proof fn lemma_view_rec_contains(self, path: TreePath<NR_ENTRIES>, m: Mapping)
        requires
            self.0.inv(),
            path.len() < INC_LEVELS - 1,
            self.view_rec(path).contains(m),
    ...
    {
        self.lemma_view_rec_contains(path, m);
        choose|i: int|
            #![auto]
            0 <= i < self.0.children.len() && self.0.children[i] is Some && PageTableOwner(
            ...
            assert(parent_ps == 512 * child_ps) by (nonlinear_arith)
                requires
                    (parent_ps == 0x1_0000_0000_0000 && child_ps == 0x80_0000_0000) || (parent_ps
                        == 0x80_0000_0000 && child_ps == 0x4000_0000) || (parent_ps == 0x4000_0000
                        && child_ps == 0x20_0000) || (parent_ps == 0x20_0000 && child_ps == 0x1000),
            ;
    ...
    pub proof fn no_frame_with_path_rec(
        ...
        requires
            ...
            forall|mm: Mapping|
                ...
                ambient.contains(mm) ==> mm.va_range.start != vaddr_of::<C>(removed_path) as int,
        ensures
            ...
            path.push_tail(i as usize), g) by {
                if self.0.value.is_node() {
                    ...
                    assert forall|mm: Mapping|
                        ...
                        self.lemma_view_rec_contains_intro(path, mm, i);
                    };
        ...
        decreases INC_LEVELS - path.len(),
    {
        ...
        if self.0.value.is_frame() {
            ...
            self.lemma_view_rec_contains(path, m1);
            self.lemma_view_rec_contains(path, m2);
            ...
            vaddr(path) + page_size((INC_LEVELS - path.len()) as PagingLevel) <= usize::MAX,
    {
        ...
        if path.len() == 0 {
            ...
            assert(s % 0x4000_0000 == 0) by (nonlinear_arith)
                requires
                    s == 0x80_0000_0000 * i0 + 0x4000_0000 * i1,
            ;
            ...
            assert(s % 0x1000 == 0) by (nonlinear_arith)
                requires
                    s == 0x80_0000_0000 * i0 + 0x4000_0000 * i1 + 0x20_0000 * i2 + 0x1000 * i3,
            ;
        ...
        decreases INC_LEVELS - path.len(),
    {
        if self.0.value.is_frame() {
            ...
            vstd::arithmetic::power2::lemma2_to64();
            vstd::arithmetic::power2::lemma2_to64_rest();
            // (A) Alignment. For `ps ∈ {2^12, 2^21, 2^30}`, `ps | 2^48`, so
            //     `lb * 2^48 % ps == 0` and `vaddr(path) % ps == 0` gives
            //     `vaddr_of(path) % ps == 0` via `lemma_mod_adds`.
            assert(vaddr(path) as int % ps == 0);
            assert(lb * 0x1_0000_0000_0000int % ps == 0) by (nonlinear_arith)
                requires
                    lb >= 0,
