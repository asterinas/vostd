...
spec fn segment_clone_accounting_transition<'rcu>(
    s_after: VmStore<'rcu>,
    old_store: VmStore<'rcu>,
    range: Range<Paddr>,
    sid2: SegmentId,
    ...
    &&& s_after.regions.inv()
    &&& s_after.frames == old_store.frames
    &&& !old_store.segments.dom().contains(sid2)
    &&& s_after.segments == old_store.segments.insert(sid2, new_entry)
    ...
    &&& forall|paddr: Paddr|
        ...
        (range.start <= paddr < range.end && paddr % PAGE_SIZE == 0) ==> {
            let idx = frame_to_index(paddr);
            let so_old = old_store.regions.slot_owners[idx];
            let so_new = s_after.regions.slot_owners[idx];
            &&& so_old.ref_count() + 1 <= REF_COUNT_MAX
    ...
    &&& forall|idx: int|
        #![trigger s_after.regions.slot_owners[idx]]
        idx < max_meta_slots() && !(range.start <= index_to_frame(idx) < range.end)
            ==> s_after.regions.slot_owners[idx] == old_store.regions.slot_owners[idx]
}

#[verifier::spinoff_prover]
proof fn lemma_segment_clone_accounting_unused<'rcu>(
    s_after: VmStore<'rcu>,
    old_store: VmStore<'rcu>,
    range: Range<Paddr>,
    sid2: SegmentId,
    ...
    requires
        segment_clone_accounting_transition(s_after, old_store, range, sid2, new_entry),
    ensures
        forall|idx: int|
            #![trigger s_after.regions.slot_owners[idx]]
            0 <= idx < max_meta_slots() && s_after.regions.slot_owners[idx].ref_count()
                == REF_COUNT_UNUSED ==> handle_count(s_after.frames, idx) == 0
                && s_after.regions.slot_owners[idx].paths_in_pt.is_empty() && segment_cover_count(
                s_after.segments,
                index_to_frame(idx),
            ) == 0,
{
    ...
    assert forall|idx: int|
        ...
        reveal(segment_clone_accounting_transition);
        reveal(VmStore::accounting_inv);
        let paddr = index_to_frame(idx);
        assert(frame_to_index(paddr) == idx);
        if range.start <= paddr < range.end {
            assert(paddr % PAGE_SIZE == 0);
            assert(s_after.regions.slot_owners[idx].ref_count() <= REF_COUNT_MAX);
            assert(false);
        } else {
            lemma_segment_cover_insert_outside(old_segments, sid2, new_entry, paddr);
        }
    };
}

#[verifier::spinoff_prover]
proof fn lemma_segment_clone_accounting_active<'rcu>(
    s_after: VmStore<'rcu>,
    old_store: VmStore<'rcu>,
    range: Range<Paddr>,
    sid2: SegmentId,
    ...
    ensures
        forall|idx: int|
            ...
            0 <= idx < max_meta_slots() && s_after.regions.slot_owners[idx].usage is Frame
                && s_after.regions.slot_owners[idx].ref_count() != REF_COUNT_UNUSED
                && s_after.regions.slot_owners[idx].ref_count() != REF_COUNT_UNIQUE
                ==> handle_count(s_after.frames, idx) > 0
                || s_after.regions.slot_owners[idx].paths_in_pt.len() > 0 || segment_cover_count(
                s_after.segments,
                index_to_frame(idx),
            ) > 0,
{
    ...
    assert forall|idx: int|
        ...
        assert(frame_to_index(paddr) == idx);
        if range.start <= paddr < range.end {
            lemma_segment_cover_insert_inside(old_segments, sid2, new_entry, paddr);
        } else {
            lemma_segment_cover_insert_outside(old_segments, sid2, new_entry, paddr);
        }
    };
...
#[verifier::spinoff_prover]
proof fn lemma_segment_clone_accounting_equation<'rcu>(
    s_after: VmStore<'rcu>,
    old_store: VmStore<'rcu>,
    range: Range<Paddr>,
    sid2: SegmentId,
    ...
    ensures
        forall|idx: int|
            ...
            handle_count(s_after.frames, idx) > 0
                ...
                &&& rc != REF_COUNT_UNUSED
                &&& rc != REF_COUNT_UNIQUE
                &&& rc == handle_count(s_after.frames, idx) + so.paths_in_pt.len()
                    + segment_cover_count(s_after.segments, index_to_frame(idx))
                &&& so.storage_perm().is_init()
            },
{
    let old_segments = old_store.segments;
...
/// Discharges the post-state accounting invariant after cloning a segment
/// range. Each slot-indexed clause gets its own solver query so their common
/// `slot_owners[idx]` trigger cannot multiply the structural proof context.
#[verifier::spinoff_prover]
proof fn lemma_step_segment_clone_range_accounting<'rcu>(
    s_after: VmStore<'rcu>,
    old_store: VmStore<'rcu>,
    range: Range<Paddr>,
    sid2: SegmentId,
    ...
    ensures
        s_after.accounting_inv(),
{
    lemma_segment_clone_accounting_unused(s_after, old_store, range, sid2, new_entry);
    lemma_segment_clone_accounting_active(s_after, old_store, range, sid2, new_entry);
    lemma_segment_clone_accounting_equation(s_after, old_store, range, sid2, new_entry);
    lemma_accounting_inv_intro(s_after);
}
        ...
        final(s).inv(),
{
    ...
    assert(segment_clone_accounting_transition(*s, s_before, sub_range, sid2, new_entry_ghost)) by {
        reveal(segment_clone_accounting_transition);
    };
    lemma_step_segment_clone_range_accounting(*s, s_before, sub_range, sid2, new_entry_ghost);
