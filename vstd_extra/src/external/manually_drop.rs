...
pub axiom fn manually_drop_into_inner_spec<T>(v: ManuallyDrop<T>, output: T)
    ensures
        call_ensures(ManuallyDrop::into_inner, (v,), output) ==> output == manually_drop_unwrap(v),
;

pub axiom fn manually_drop_new_deref_spec<T>(v: T, output: ManuallyDrop<T>)
    ensures
        call_ensures(ManuallyDrop::new, (v,), output)
