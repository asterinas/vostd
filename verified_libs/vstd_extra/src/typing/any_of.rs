...
//!
//! Writing an aggregate by hand means writing, per member, a [`TypeSet`] impl and a
...
//! # Nesting direction
//!
//! The tree is built *left*-nested — `ConsType<ConsType<LeafType<A>, LeafType<B>>,
//! LeafType<C>>` — for a mechanical reason rather than a semantic one. Both
//! directions are equivalent (`possible_types` and `valid` are unions, and union
//! is associative), but a
...
//!
//! Ids live in the member's own `TypeSet` impl, so a type cannot appear in two
...
//!
//! Nothing about representation. `LeafType<M>` is a [`Member`] only when `M` is
...
//! ```
//!
//! expands to `TypeSet`/`HasId` for each of the three, two `DisjointFrom`
//! witnesses, and `pub type World = ConsType<ConsType<LeafType<MetaA>,
//! LeafType<MetaB>>, LeafType<MetaC>>`.
/// Defines a closed world of types as a nested `EitherType` tree.
///
...
macro_rules! AnyOf {
    ...
    ($world:ident = [$t0:ty = $id0:literal $(, $t:ty = $id:literal)* $(,)?]) => {
        ...
        $crate::AnyOf!(@tree $world; $crate::typing::types::LeafType<$t0> $(; $t)*);
    };
    ...
    (@ids $($t:ty = $id:literal),+) => {
        ::vstd::prelude::verus!{
        $(
        impl $crate::typing::types::TypeSet for $t {
            open spec fn possible_types() -> ::vstd::set::Set<nat> {
                ::vstd::set::Set::empty().insert($id as nat)
            }
        }
    ...
    (@tree $world:ident; $acc:ty ; $next:ty $(; $rest:ty)*) => {
        ::vstd::prelude::verus!{
        impl $crate::typing::types::DisjointFrom<$crate::typing::types::LeafType<$next>> for $acc {
...
$crate::AnyOf!(
            @tree $world;
            $crate::typing::types::ConsType<$acc, $crate::typing::types::LeafType<$next>>
            $(; $rest)*
        );
    };
}
