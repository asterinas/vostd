...
pub unsafe trait AnyMeta: Send + Sync {
    ...
    spec fn type_id(&self) -> TypeIdSpec;

    /// The executable form, dispatched through the vtable.
    fn type_id_val(&self) -> (r: TypeId)
        ensures
            r.view() == self.type_id(),
    ;
    ...
    ///
    /// This is the whole of what `MetaTag` used to be. That trait gave every
    /// implementor a hand-chosen `usize` and then required, by hand, that the
    /// value's dispatched id agree with it -- its own doc called this "the fact
    /// `Any` provides for free and the one thing that has to be supplied by hand".
    /// It is now supplied for free: the body is empty because `type_id::<Self>()`
    /// is exactly what `type_id` is obliged to return.
    proof fn type_id_coherent(&self) where Self: core::marker::Sized
        ensures
            self.type_id() == type_id::<Self>(),
    ;
...
unsafe impl AnyMeta for MetaA {
    open spec fn type_id(&self) -> TypeIdSpec {
        type_id::<MetaA>()
    }
    ...
    proof fn type_id_coherent(&self) {
    }
...
impl Frame<dyn AnyMeta> {
    ...
    pub uninterp spec fn meta_id(&self) -> TypeIdSpec;
...
///
/// ```text
/// if dyn_frame.dyn_meta().type_id_val().eq(&TypeId::of::<M>()) {
/// ```
///
/// Both compare an id read through the vtable against one known statically, and
/// both ids are now the *same notion of identity* -- upstream reads it with
/// `TypeIdSpec::of`, we read it with `type_id::<M>()`. Previously the right-hand side
/// was a hand-chosen `usize` from a `MetaTag` impl, related to the real identity
/// only by an obligation each implementor discharged by hand.
///
/// A free function rather than a `TryFrom` impl, to keep the plumbing visible;
...
pub fn try_from_tagged<M: AnyMeta + 'static>(dyn_frame: Frame<dyn AnyMeta>) -> (res: Result<
    Frame<M>,
    Frame<dyn AnyMeta>,
>)
    ensures
        (res is Ok) == (dyn_frame.meta_id() == type_id::<M>()),
{
    if dyn_frame.dyn_meta().type_id_val().eq(&TypeId::of::<M>()) {
...
pub fn downcast_discriminates(a: Frame<dyn AnyMeta>, b: Frame<dyn AnyMeta>)
    requires
        a.meta_id() == type_id::<MetaA>(),
        b.meta_id() == type_id::<MetaB>(),
{
