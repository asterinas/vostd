...
impl<V> HistAuth<V> {
    ...
    pub open spec fn history(self) -> History<V>
        recommends
            self.wf(),
    {
...
macro_rules! declare_weak_atomic_type {
    ($weak_atomic:ident, $pred_adapter:ident, $raw_atomic:ident, $value_ty:ty) => {
        ...
        impl<K, G, Pred> $weak_atomic<K, G, Pred> {
            ...
            /// Logical modification-history identity of this atomic.
            pub closed spec fn id(&self) -> AtomicId {
            ...
            /// Borrows the executable atomic for a client-specific invariant
            /// transition.
            #[doc(hidden)]
            pub fn raw_atomic(&self) -> (res: &$raw_atomic)
                requires
                    self.well_formed(),
                ensures
                    res.id() == self.id(),
            ...
            /// Borrows the invariant for a client-specific atomic operation.
            ///
            /// The fields remain private so the type invariant cannot be
            /// bypassed by construction or replacement.
            #[doc(hidden)]
            pub proof fn tracked_atomic_inv(
                tracked &self,
            ) -> (tracked res: &AtomicInvariant<
                (K, AtomicId),
                (HistAuth<$value_ty>, G),
                $pred_adapter<Pred>,
            >)
...
impl<T, K, G, Pred> WeakAtomicPtr<T, K, G, Pred> {
    ...
    /// Borrows the executable pointer atomic for a client-specific invariant
    /// transition.
    #[doc(hidden)]
    pub fn raw_atomic(&self) -> (res: &AtomicPtrW<T>)
        requires
            self.well_formed(),
        ensures
            res.id() == self.id(),
    ...
    /// Borrows the invariant for a client-specific atomic operation.
    #[doc(hidden)]
    pub proof fn tracked_atomic_inv(tracked &self) -> (tracked res: &AtomicInvariant<
        (K, AtomicId),
        (HistAuth<*mut T>, G),
        WeakAtomicPredPtr<T, Pred>,
    >)
        requires
            self.well_formed(),
        ensures
...
macro_rules! weak_atomic_with_ghost {
    (
        ...
        ghost $g:ident => $b:block
            ...
            let raw_atomic = atomic.raw_atomic();
            ::vstd::invariant::open_atomic_invariant!(atomic.tracked_atomic_inv() => pair => {
                #[allow(unused_mut)]
                let tracked (mut hist, mut $g) = pair;
                proof {
                    assert(hist.id() == atomic.id());
                    assert(raw_atomic.id() == atomic.id());
    ...
    (
        ...
        ghost $g:ident => $b:block
            ...
            ::vstd::invariant::open_atomic_invariant!(atomic.tracked_atomic_inv() => pair => {
                ...
                result = raw_atomic.load_acquire(Tracked(&hist), $tv);
    ...
    (
        ...
        ghost $g:ident => $b:block
            ...
            ::vstd::invariant::open_atomic_invariant!(atomic.tracked_atomic_inv() => pair => {
                ...
                let ghost $history = hist.history();
                result = raw_atomic.load_relaxed(Tracked(&hist), $tv);
                let ghost $ret = result.0;
                let ghost $ts = result.1@;
                let ghost $msg = hist.msg_at($ts);
    ...
    (
        ...
        ghost $g:ident => $b:block
            ...
            ::vstd::invariant::open_atomic_invariant!(atomic.tracked_atomic_inv() => pair => {
                ...
                let ghost $prev = hist.history();
                let snap_tracked = raw_atomic.store_release(Tracked(&mut hist), $tv, value);
                let ghost $next = hist.history();
    ...
    (
        ...
        ghost $g:ident => $b:block
            ...
            ::vstd::invariant::open_atomic_invariant!(atomic.tracked_atomic_inv() => pair => {
                ...
                let ghost $prev = hist.history();
                let snap_tracked = raw_atomic.store_relaxed(Tracked(&mut hist), $tv, value);
                let ghost $next = hist.history();
...
pub use weak_atomic_with_ghost;
...
impl ThreadView {
    ...
    ///
    /// This generic constructor is public because schedulers live outside
    /// `vstd_extra`. Minting a fresh empty view is sound but loses ordering
    /// knowledge. Production OSTD code therefore calls it only when registering
    /// a task or CPU, then moves that same linear token through schedule-in and
    /// schedule-out.
